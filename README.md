...
而"本地缓存"页要反向枚举出曲名、UP主、音质与体积。所以解析成功时会在缓存目录里顺手
写一份 `index.json`(纯 JSON,原子写入):
...
```json
{
  ...
  "tracks": [
    {
      "file_name": "45b84aaacfaedca1ffc0.m4a",
      "bvid": "BV1xx411c7mD",
      "cid": 123456,
      "title": "晴天",
      "author": "周杰伦",
      ...
      "duration": 269,
      "size_bytes": 6451200,
      "cached_at": 1757700000.0
    }
  ]
}
```

设计要点(实现见 `core/cache_index.py`):

- **索引是可再生的**:读坏、丢失、被手删,都会退回按档位盲查(`pick_best_cached`),
  绝不影响播放;写索引失败也只当这次记录作废,不会让一次已经成功的播放变成失败。
- **单个文件而不是每个音频配一个 sidecar**:枚举只读一次盘,增删也只写一次盘,
  缓存目录里也不会散出成百上千个小文件。
- **`file_name` 只接受纯文件名**:索引文件在用户目录里、可以被手改,这一条能挡住
  `..\..\x.m4a` 这类路径穿越 —— 否则一次"删除缓存"会删到缓存目录之外。
- **重复播放不刷新缓存时间**:`cached_at` 记的是"这首歌什么时候存下来的",
  内容没变时连落盘都跳过(命中缓存的那条路径同样会调用它,所以不会重复写)。
...
- [x] **左侧导航栏**:搜索结果页与本地缓存页是真的;"发现"与"我的歌单"给出**说明清楚的占位页**
      (功能分别属于路线图 M3 / M5,还没有实现)
...
- [x] **缓存索引**(与音频文件同目录的 `index.json`):记下每首已缓存音轨的曲名 / UP主 /
      音质 / 时长 / 体积,让缓存**可枚举**。索引读坏或丢失只会退回按档位盲查,不影响播放
...
- [ ] 本地曲库的进阶能力:下载队列、整合集一键缓存、断点续传(路线图 M2.3–M2.5)
