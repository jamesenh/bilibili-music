...
侧栏里"发现"与"我的歌单"这些入口在设计稿上有位置,但对应功能分别属于路线图的
M3 / M5,**当前一律没有实现**(``AGENTS.md`` 第 1.4 节:不在路线图内的不自行扩范围)。
做成"点了没反应"或者干脆不画,用户只会以为程序坏了;这里给一个明确的占位页:说清楚
这一页叫什么、为什么还是空的。

(`搜索结果` / `最近播放` / `本地缓存`三页都是真的,不走这里。)
